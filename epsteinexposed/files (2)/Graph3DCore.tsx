...
const NODE_COLORS: Record<string, { main: string; glow: string }> = {
  person: { main: '#00d4ff', glow: 'rgba(0, 212, 255, 0.6)' },       // Cyan
  location: { main: '#ffb800', glow: 'rgba(255, 184, 0, 0.6)' },     // Amber
  organization: { main: '#ff3366', glow: 'rgba(255, 51, 102, 0.6)' }, // Red
  date: { main: '#00ff88', glow: 'rgba(0, 255, 136, 0.6)' },         // Green
};
...
function GraphNode({
  ...
  // Calculate node size based on document count (logarithmic scale)
  ...
  useFrame((state, delta) => {
    ...
    // Glow pulse effect for selected/multi-selected
    if (glowRef.current) {
  ...
  return (
    <group position={node.position}>
      ...
      {/* Inner selection ring for single select */}
      ...
      {/* Label - show on hover or selection */}
      {(isHovered || isSelected || isMultiSelected) && (
        <>
          <Text
            position={[0, baseSize * 2 + 0.3, 0]}
            fontSize={0.15}
            color="#ffffff"
            anchorX="center"
            anchorY="middle"
...
function CameraController({ target }: { target: [number, number, number] | null }) {
  ...
  useFrame(() => {
    if (controlsRef.current && target) {
      ...
      controls.target.lerp(new THREE.Vector3(...target), 0.02);
      controls.update();
    }
  });
...
export function Graph3DCore({
  ...
  useEffect(() => {
    const fetchGraph = async () => {
      try {
        ...
        if (data.result?.data) {
          // Add 3D positions if not present (force-directed layout placeholder)
          const nodesWithPositions = data.result.data.nodes.map((node: any, i: number) => {
            if (node.position) return node;
            
            // Sphere distribution for initial layout
  ...
  // Handle node click with CTRL detection for multi-select
  ...
  // Handle analyze connection button click
  ...
  // Calculate highlighted edges (connected to selected/multi-selected nodes)
  ...
  // Camera target (focus on selected node)
    ...
    return (
      <div className="w-full h-full flex items-center justify-center bg-[#0a0a0f]">
        <div className="text-center">
          ...
          <p className="font-mono text-[#00d4ff] text-sm">
            INITIALIZING NEURAL GRAPH...
          </p>
          <p className="font-mono text-[#606070] text-xs mt-2">
            Loading {graphData.nodes.length || '...'} entities
          </p>
        </div>
        <style jsx>{`
          @keyframes scan {
      ...
      <div className="w-full h-full flex items-center justify-center bg-[#0a0a0f]">
        <div className="text-center">
          ...
          <button
            ...
            className="px-4 py-2 bg-[#ff3366]/20 text-[#ff3366] rounded font-mono text-sm hover:bg-[#ff3366]/30"
          >
            Retry
          </button>
        </div>
          ...
          <div className="bg-[#12121a]/95 backdrop-blur-sm border border-[#00d4ff]/30 rounded-lg px-5 py-3 shadow-xl">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[#00d4ff] font-mono text-sm font-medium">
              ...
              <span className="text-[#606070] font-mono text-xs">
              ...
              {multiSelectedNodes.size >= 2 && (
                <button
                  ...
                  className="px-4 py-1.5 bg-[#00d4ff] text-[#0a0a0f] rounded font-mono text-xs font-bold
                           hover:bg-[#00d4ff]/90 transition-colors flex items-center gap-2"
                >
              ...
              <button
                ...
                className="p-1.5 text-[#606070] hover:text-[#ff3366] transition-colors"
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {[...multiSelectedNodes].map(nodeId => {
                const node = graphData.nodes.find(n => n.id === nodeId);
                const colors = NODE_COLORS[node?.type || 'person'];
                return (
                  <span
                    key={nodeId}
