...
export function AnomalyStream() {
  ...
  const handleViewDocuments = (anomaly: Anomaly, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleRelatedEntities = (anomaly: Anomaly, e: React.MouseEvent) => {
    e.stopPropagation();
                    ...
                    {anomaly.expanded && (
                      ...
                      >
                        <div className="text-xs text-gray-300 space-y-2">
                          ...
                          {/* Entity tags */}
                          {anomaly.entities && anomaly.entities.length > 0 && (
                            <div className="flex flex-wrap gap-1 mt-2">
                              {anomaly.entities.slice(0, 5).map((entity, i) => (
                                <span key={i} className="px-1.5 py-0.5 bg-amber-500/10 text-amber-400 rounded text-xs">
                                  {entity}
                                </span>
                              ))}
                              {anomaly.entities.length > 5 && (
                                <span className="text-gray-500 text-xs">
                                  +{anomaly.entities.length - 5} more
                                </span>
                              )}
                            </div>
                          )}
                          ...
                          <div className="flex gap-2 mt-3">
                            <button 
                              ...
                              onClick={(e) => handleViewDocuments(anomaly, e)}
                            >
                            ...
                            <button 
                              ...
                              onClick={(e) => handleRelatedEntities(anomaly, e)}
                            >
                              Highlight in Graph
                            </button>
                          </div>
